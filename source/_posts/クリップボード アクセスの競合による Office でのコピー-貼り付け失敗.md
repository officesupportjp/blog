...
エクスプローラのリボンに \[貼り付け\] ボタンがあります。コピーデータがファイルなどのエクスプローラ上で貼り付けられるデータの場合に、このリボンのボタンを活性化するため、リボンが展開表示されている場合は、コピー時にエクスプローラからもアクセスが発生します。

Windows 10 では、リボンを閉じた状態で OS を再起動してそのままリボンを開かなければ、このアクセスは発生しません。  
Windows 11 では、貼り付けボタンが常時表示に変更され、このアクセスを止める方法はありません。エクスプローラー ウィンドウ (タブ) を多く開いているとアクセスが増加するため、不要なエクスプローラー ウィンドウ (タブ) を閉じることで軽減できます。
...
Microsoft 365 Apps の例  
![](M365Err.png)

貼り付け時のクリップボード アクセス失敗の場合は、クリップボードからデータを取得できないため貼り付けられません。この時は、特にエラーメッセージは表示されません。  

補足) Excel でエラーメッセージを表示しない方法  
Microsoft 365 Apps の新しいバージョンでは、メッセージ内の [今後表示しない] にチェックを入れるか、オプションやレジストリでこのメッセージを表示しないようにできます。
Excel 内でのコピー / 貼り付けが主な利用シナリオのため通知が不要な場合には、必要に応じてメッセージの表示を抑止できます。(競合の発生を防ぐものではありません。メッセージ表示のみの制御です。)

<オプション>  
Excel の [オプション] - [詳細設定] - [切り取り、コピー、貼り付け] - [外部クリップボード操作が失敗した場合に警告する] のチェックをオフ
...
※ 先にレジストリを追加し、その後のバージョンでオプションを追加しました。ご利用のバージョンでオプションが存在しない場合は、レジストリをお試しください。
<br>

...
*   シングルサインオン環境が必須であること  
    記事内では "ライセンス トークンの移動" について触れられていますが、"ライセンストークンの移動" を実施しても、次回 OS にログオンし M365Apps を起動すると、右上のサインインユーザーや [ファイル] - "アカウント" 画面に警告が表示されたり、サインイン画面が表示されたりします。  
    
    ![右上のサインインユーザーの警告](image3.png)  
    
    ![[ファイル] - "アカウント" 画面の警告](image2.png)  

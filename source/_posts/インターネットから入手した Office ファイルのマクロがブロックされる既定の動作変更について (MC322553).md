...
本記事では、2022 年 4 月 より Microsoft 365 Apps への展開が開始されるインターネットから入手した Office ファイルのマクロがブロックされる既定の動作変更について説明します。この情報は、MC322553 で通知されているものです。
...
MOTW 属性が付与されていることは、ファイルのプロパティ画面で確認できます (後述の "Web マークを削除する方法" に実際の画面イメージを掲載します)。
...
- インターネット ゾーンの Web サイト上のファイルを直接開く
...
## 4) Web 上のファイルを開くシナリオで、信頼済みサイトに登録する方法
自社 Web サイトなど安全が保証されているサイトの Office ファイルを開く場合は、Web サイトを信頼済みサイトに登録すると、インターネット入手ファイルとみなされずマクロを有効にできます。
...
1. マクロの実行を許可する Office ファイルが格納されたサイトの URL を入力して [追加] をクリックします。 
